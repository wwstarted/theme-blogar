...
function blogar_register_settings()
{
    ...
    add_settings_section('blogar_s11_section', __('News Highlight Block [v2]', 'blogar'), 'blogar_s11_section_cb', 'blogar-s11');
    ...
    add_settings_field('blogar_s11_ticker_title', __('[v2] Trending label', 'blogar'), 'blogar_text_field_cb', 'blogar-s11', 'blogar_s11_section', array(
        'option_name' => 'blogar_s11_ticker_title',
        'default' => 'Trending Now',
        'placeholder' => 'Trending Now',
        'description' => __('Label shown above the trending text list in the right column (e.g. "TRENDING NOW").', 'blogar'),
    ));
    ...
    add_settings_field('blogar_s11_main_post', __('[v2] Hero post (big featured)', 'blogar'), 'blogar_post_select_field', 'blogar-s11', 'blogar_s11_section', array(
        'option_name' => 'blogar_s11_main_post',
        'description' => __('[v2] Large full-image card on the left (50% column). Falls back to the latest post.', 'blogar'),
    ));

    // [v2] small_post_1 = middle card top, small_post_2 = middle card bottom
    $s11_mid_labels = array(
        1 => __('[v2] Middle card 1 — top', 'blogar'),
        2 => __('[v2] Middle card 2 — bottom', 'blogar'),
    );
    for ($i = 1; $i <= 2; $i++) {
        add_settings_field("blogar_s11_small_post_{$i}", $s11_mid_labels[$i], 'blogar_post_select_field', 'blogar-s11', 'blogar_s11_section', array(
            'option_name' => "blogar_s11_small_post_{$i}",
            'description' => $i === 1 ? __('[v2] Two stacked full-image cards in the center column (25%). Falls back to latest posts.', 'blogar') : '',
        ));
    }

    // ── Section s11p: Penci Featured Layout (faithful clone) ────────
    register_setting('blogar_s11p_settings', 'blogar_s11p_enabled', array('sanitize_callback' => 'absint', 'default' => 1));
    register_setting('blogar_s11p_settings', 'blogar_s11p_trending_label', array('sanitize_callback' => 'sanitize_text_field'));
    register_setting('blogar_s11p_settings', 'blogar_s11p_cat', array('sanitize_callback' => 'absint'));
    register_setting('blogar_s11p_settings', 'blogar_s11p_hero', array('sanitize_callback' => 'absint'));
    register_setting('blogar_s11p_settings', 'blogar_s11p_mid1', array('sanitize_callback' => 'absint'));
    register_setting('blogar_s11p_settings', 'blogar_s11p_mid2', array('sanitize_callback' => 'absint'));
    register_setting('blogar_s11p_settings', 'blogar_s11p_slider_count', array('sanitize_callback' => 'absint', 'default' => 3));
    register_setting('blogar_s11p_settings', 'blogar_s11p_trending_cat', array('sanitize_callback' => 'absint'));

    for ($i = 2; $i <= 6; $i++) {
        register_setting('blogar_s11p_settings', "blogar_s11p_slider_post_{$i}", array('sanitize_callback' => 'absint'));
    }
    for ($i = 1; $i <= 4; $i++) {
        register_setting('blogar_s11p_settings', "blogar_s11p_left_small_{$i}", array('sanitize_callback' => 'absint'));
    }
    for ($i = 1; $i <= 3; $i++) {
        register_setting('blogar_s11p_settings', "blogar_s11p_right_small_{$i}", array('sanitize_callback' => 'absint'));
    }

    add_settings_section('blogar_s11p_section', __('Penci Featured Layout [clone]', 'blogar'), 'blogar_s11p_section_cb', 'blogar-s11p');

    add_settings_field('blogar_s11p_enabled', __('Display section', 'blogar'), 'blogar_section_toggle_field_cb', 'blogar-s11p', 'blogar_s11p_section', array('option_name' => 'blogar_s11p_enabled'));

    add_settings_field('blogar_s11p_cat', __('Fallback category', 'blogar'), 'blogar_category_select_field', 'blogar-s11p', 'blogar_s11p_section', array(
        'option_name' => 'blogar_s11p_cat',
        'description' => __('Used only for empty slots. If a slot has no selected post, it auto-fills from this category; otherwise it falls back to the latest posts.', 'blogar'),
    ));

    add_settings_field('blogar_s11p_slider_count', __('Slider post count', 'blogar'), 'blogar_number_field_cb', 'blogar-s11p', 'blogar_s11p_section', array(
        'option_name' => 'blogar_s11p_slider_count',
        'default' => 3,
        'min' => 1,
        'max' => 6,
        'description' => __('How many posts appear in the main slider on the left column (1–6).', 'blogar'),
    ));

    add_settings_field('blogar_s11p_hero', __('Slider post 1 (first slide)', 'blogar'), 'blogar_post_select_field', 'blogar-s11p', 'blogar_s11p_section', array(
        'option_name' => 'blogar_s11p_hero',
        'description' => __('Main slider on the left column. Pick posts in display order; empty slots auto-fill from the fallback category or latest posts.', 'blogar'),
    ));
    for ($i = 2; $i <= 6; $i++) {
        add_settings_field("blogar_s11p_slider_post_{$i}", sprintf(__('Slider post %d', 'blogar'), $i), 'blogar_post_select_field', 'blogar-s11p', 'blogar_s11p_section', array(
            'option_name' => "blogar_s11p_slider_post_{$i}",
            'description' => '',
        ));
    }

    for ($i = 1; $i <= 4; $i++) {
        add_settings_field("blogar_s11p_left_small_{$i}", sprintf(__('Left small box %d', 'blogar'), $i), 'blogar_post_select_field', 'blogar-s11p', 'blogar_s11p_section', array(
            'option_name' => "blogar_s11p_left_small_{$i}",
            'description' => $i === 1 ? __('The 4 small cards below the slider, ordered from left to right and top to bottom.', 'blogar') : '',
        ));
    }

    add_settings_field('blogar_s11p_mid1', __('Middle card (center 25%)', 'blogar'), 'blogar_post_select_field', 'blogar-s11p', 'blogar_s11p_section', array(
        'option_name' => 'blogar_s11p_mid1',
        'description' => __('Big-grid image card in the center column, above trending list.', 'blogar'),
    ));

    add_settings_field('blogar_s11p_trending_label', __('Trending label', 'blogar'), 'blogar_text_field_cb', 'blogar-s11p', 'blogar_s11p_section', array(
        'option_name' => 'blogar_s11p_trending_label',
        'default' => 'Trending Now',
        'placeholder' => 'Trending Now',
        'description' => __('Label for the trending list in the middle column.', 'blogar'),
    ));

    add_settings_field('blogar_s11p_trending_cat', __('Trending category', 'blogar'), 'blogar_category_select_field', 'blogar-s11p', 'blogar_s11p_section', array(
        'option_name' => 'blogar_s11p_trending_cat',
        'default' => (int) get_option('blogar_s11p_cat', 0),
        'description' => __('Category source for the "Trending Now" list only. Leave empty to use the fallback category or latest posts.', 'blogar'),
    ));

    add_settings_field('blogar_s11p_mid2', __('Right card (right 25%)', 'blogar'), 'blogar_post_select_field', 'blogar-s11p', 'blogar_s11p_section', array(
        'option_name' => 'blogar_s11p_mid2',
        'description' => __('Big-grid image card in the right column, above the small list.', 'blogar'),
    ));

    for ($i = 1; $i <= 3; $i++) {
        add_settings_field("blogar_s11p_right_small_{$i}", sprintf(__('Right small box %d', 'blogar'), $i), 'blogar_post_select_field', 'blogar-s11p', 'blogar_s11p_section', array(
            'option_name' => "blogar_s11p_right_small_{$i}",
            'description' => $i === 1 ? __('The 3 small cards in the right column, below the big image card.', 'blogar') : '',
        ));
    }
    ...
    // ── Section 15: Entertainment Category Slider ────────────────
    $legacy_s15_settings = get_option('blogar_s15_settings', array());

    register_setting('blogar_s15_settings', 'blogar_s15_enabled', array('sanitize_callback' => 'absint', 'default' => 1));
    ...
    add_settings_field('blogar_s15_enabled', __('Display section', 'blogar'), 'blogar_section_toggle_field_cb', 'blogar-s15', 'blogar_s15_section', array('option_name' => 'blogar_s15_enabled'));

    add_settings_field('blogar_s15_title', __('Section title', 'blogar'), 'blogar_text_field_cb', 'blogar-s15', 'blogar_s15_section', array(
        'option_name' => 'blogar_s15_title',
        'default' => !empty($legacy_s15_settings['title']) ? $legacy_s15_settings['title'] : 'Entertainment',
...
function blogar_s11_section_cb()
{
    echo '<p class="description">'
        . esc_html__('[v2 — Penci layout] 3-column block: (50%) big hero image | (25%) 2 stacked photo cards | (25%) trending text list + small cards. Configure the hero, 2 middle cards and the trending label. Small horizontal cards and the trending list are auto-filled from the latest posts.', 'blogar')
        . '</p>';
}

function blogar_s11p_section_cb()
{
    echo '<p class="description">'
        . esc_html__('Penci-style asymmetric layout with exact slot control. You can assign posts for each slider slide, the 4 left small cards, the 2 big image cards, and the 3 right small cards. The "Trending Now" list pulls by category only, while empty slots auto-fill from the fallback category or latest posts.', 'blogar')
        . '</p>';
}
...
function blogar_section_toggle_field_cb($args)
{
    ...
    <label style="display:flex;align-items:center;gap:10px;cursor:pointer;font-size:13px;font-weight:500;">
        ...
        <input type="checkbox" id="<?php echo esc_attr($option); ?>" name="<?php echo esc_attr($option); ?>" value="1" <?php checked($enabled, true); ?>>
...
function blogar_category_select_field($args)
{
    ...
    $selected_id = (int) get_option($option_name, $default);
    $description = isset($args['description']) ? $args['description'] : '';
...
function blogar_render_options_page()
{
    ...
    $tabs = array(
        ...
        's11p_penci_featured' => array(
            'label' => __('① Penci Featured', 'blogar'),
            'settings_group' => 'blogar_s11p_settings',
            'page_slug' => 'blogar-s11p',
        ),
        's5_topics' => array(
        ...
        's15_category_slider' => array(
            'label' => __('⑧ Entertainment Slider', 'blogar'),
...
function blogar_render_order_tab()
{
    $sections_meta = array(
        ...
        's11p' => array('label' => __('① Penci Featured', 'blogar'), 'desc' => __('Penci-style 3-col: hero + big-grid + trending', 'blogar')),
            ...
            <?php foreach ($current_order as $key):
                ...
                <li data-key="<?php echo esc_attr($key); ?>" draggable="true"
                    ...
                    <label
                        ...
                        <input type="checkbox" name="blogar_sidebar_<?php echo esc_attr($key); ?>_enabled" value="1" <?php checked($enabled); ?>>
                        <?php esc_html_e('Hiện', 'blogar'); ?>
                    </label>
        ...
        <table class="form-table" role="presentation">
            ...
            <tr>
                ...
                <td>
                    ...
                    <p class="description">
                        <?php esc_html_e('Tiêu đề lớn hiển thị trên trang 404. Để trống dùng mặc định.', 'blogar'); ?>
                    </p>
                </td>
            </tr>
                            ...
                            <?php if (!empty($cats)): ?>
                                <optgroup label="<?php esc_attr_e('── Danh mục', 'blogar'); ?>">
                                    <?php foreach ($cats as $cat): ?>
                                        <option value="<?php echo absint($cat->term_id); ?>" <?php selected($sel[$i], $cat->term_id); ?>>
                                            <?php
                                            printf(
                                                '%s (%d %s)',
                                                esc_html($cat->name),
